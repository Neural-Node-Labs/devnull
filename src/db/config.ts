 ...
 * - SQLite path: ~/.devnull/data/devnull.db
 * - Postgres: localhost:5432, devnull/devnull_pass, database=devnull
 */
export interface DatabaseConfig {
  ...
  // ─── SQLite-specific ──────────────────────────────────────────────────
  /** Path to the SQLite database file. Default: ~/.devnull/data/devnull.db */
  sqlitePath: string;
...
/**
 * Default SQLite database path: ~/.devnull/data/devnull.db
 * Creates the directory if it doesn't exist.
 */
function defaultSqlitePath(): string {
  const home = process.env.DEVNULL_HOME || os.homedir();
  const dbDir = path.join(home, ".devnull", "data");
  ...
  return path.join(dbDir, "devnull.db");
}
 ...
 *   DATABASE_SQLITE_PATH   = custom SQLite file path (default: ~/.devnull/data/devnull.db)
