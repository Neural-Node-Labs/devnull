...
const PROTOCOL_PATH = path.join("agent", "devnull.md");
const LESSONS_PATH = path.join("tasks", "lessons.md");
const TODO_PATH = path.join("tasks", "todo.md");

/** Reads agent/devnull.md if present; this is the engineering protocol from the workflow doc. */
export function loadProtocol(cwd: string = process.cwd()): string | undefined {
  const p = path.join(cwd, PROTOCOL_PATH);
  if (fs.existsSync(p)) return fs.readFileSync(p, "utf-8").trim();

  // Fall back to the protocol baked into the devnull install (Docker image) if the workspace
  // doesn't ship its own -- same reasoning as SkillRegistry's DEVNULL_HOME fallback.
